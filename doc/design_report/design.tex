...
\includegraphics[width=\textwidth]{../images/sig_sheet.png}
\newpage

\parskip = 0.30\baselineskip
\tableofcontents
...
language (Verilog). We were able to achieve full system level simulation and
proved that our system has identical execution to an emulator which we used as
our known `good' system to compare against. In the end we were successfully
able to run original game ROMs on an Altera DE1 FPGA development board proving
our project to be a success.
...
maintained similar to software. This allows you to future proof the existing
design and even extend its functionality. This project is a case study on
re-implementing an old game console, the Sega Game Gear, in a FPGA.
...
memory management unit that selects between the two. After that we were ready
...
\begin{tabular}{L{1.75in}L{2.75in}C{0.6in}C{0.7in}}
    ...
    Memory Multiplexer      & Memory mux must be able to properly select between the cartridge, RAM, and IO devices &{\color{darkgreen} Yes}     & {\color{darkgreen} Yes} \\ 
    \bottomrule
\end{tabular}
\rowcolors{1}{}{}
...
\subsection{Sega Game Gear Overview}
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.4]{../images/gg.jpg}
    \caption{Sega Game Gear \protect\cite{gg}}
    \label{fig:gg}
\end{figure}

\subsection{Altera DE1 Overview}

Our target platform for our re-implementation is the Altera DE-1 development
board which contains a number of features such as a Cyclone II EP2C20F484C7,
512 KB SRAM, 4 MB Flash, VGA 4-bit DAC, and a SD card slot. There is nothing in
particular about this board that makes it more suitable for our project than
any other FPGA development board in the same class. It was chosen purely
because we already owned them and have used them in the past.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{../images/de1.jpg}
    \caption{Altera DE1 \protect\cite{de1}}
    \label{fig:de1}
\end{figure}
...
physical components found in the original Game Gear. Additional components not
found in the original system such as an IO controller and a memory management
unit (MMU) are added to account for the lack of tri-state buses in our design
(explained later).  Figure~\ref{fig:internal} shows the interactions between
the major functional components in Sega Game Gear. The colored components are
those which we implemented in our project.
...
The Z80 CPU only has 64KB of address space 48KB of which is dedicated to the
...
The \texttt{INT} bit indicates a frame interrupt is pending and is set on the
...
\texttt{OVR} and \texttt{COL} bits are used for sprites and are not implemented
...
The VDP is responsible for both a Horizontal and Vertical counter.
...
The horizontal and vertical counters can be read by the Z80 through the two IO
ports described earlier. Our design implements the standard Game Gear NTSC
display size of 256x192. The vertical counter is not completely linear. While
the NTSC display size is described as 192 lines there is actually 262 lines
when blanking is included.  For each of the 262 lines the vertical counter increments
...
\begin{table}[H]
    ...
    \begin{tabular}{cc}
        \toprule
        \textbf{V Counter Values} & \textbf{Line numbers} \\
        \midrule
        \texttt{0x00 - 0xDA} & \texttt{0 - 218} \\
        \texttt{0xD5 - 0xFF} & \texttt{219 - 261} \\
        \bottomrule
    \end{tabular}
    \caption{Vertical Counter Values}
\end{table}

A table showing the number of scanlines for each event is shown below:

\begin{table}[H]
    \centering
    \begin{tabular}{cl}
        \toprule
        \textbf{Number of Scanlines} & \textbf{Event} \\
        \midrule
        192     & Active Display    \\
        24      & Bottom Border     \\
        3       & Bottom Blanking   \\
...
\vspace{\baselineskip}
\textbf{Frame Interrupts}
\vspace{0.5\baselineskip}

The VDP support several different display modes which determine the height of the display.
Depending on the display  mode the VDP will generate an interrupt on a particular scanline.
The table below shows which line the VDP generates an interrupt on for each of the three possible
display heights.
...
\begin{table}[H]
    ...
    \begin{tabular}{cc}
        \toprule
        \textbf{Display Height}  & \textbf{Scanline} \\
        \midrule
        192     & 0xC1 \\
        224     & 0xE1 \\
        240     & 0xF1 \\
...
Depending on the height (fixed at 192 in our design) an event is raised at the
beginning of scanline 193 (0xC1). This event sets bit 7 of the status flag
register to indicate a frame interrupt request. This bit will remain set until
the control port is read.

The 5th bit of the register 0 acts like an enable for the VDP's interrupt
request (IRQ) line. The bits in the status register indicate an interrupt
request. The VDP will only assert the IRQ line if an interrupt is requested and
the 5th bit is set to a 1. The VDP will de-assert the IRQ line if either of
those conditions is false.

\vspace{\baselineskip}
\textbf{Line Interrupts}
\vspace{0.5\baselineskip}

The contents of the 10th register is loaded into a counter on every line
outside of the active display period which excludes the line after the last
line of the active display period. The counter is then decremented on every
line within the active display period including the line after the last line of
the active display period.

For example, with our design which has 192 display lines and a total of 262
scanlines per frame the counter is decremented on lines 0-192 and the counter
is reloaded with the contents of register 10 on lines 193-261.

If the counter underflows from 0 to 255, then it is reloaded with the previous
contents stored in register 10. Writing to the 10th register will not
immediately affect/change the contents of the counter; this only occurs when
the VDP is outside of the active display or when (counter is reloaded) or when
the counter underflows.

The VDP sets an internal flag when the counter underflows. This flag remains
set until the control port is read.

The 4th bit of register 0 acts like an enable for the VDP's IRQ line.
If the internal flag is set and the 4th bit of register 0 is set, then the
VDP will assert the IRQ line, and will de-assert the IRQ line if the bit is
cleared.
...
\begin{figure}[H]
    ...
    \begin{minipage}[H]{0.45\linewidth}
        \centering
        \includegraphics[width=0.8\textwidth]{../images/tiles_screen.png}
    ...
    \begin{minipage}[H]{0.45\linewidth}
        \centering
        \includegraphics[width=0.8\textwidth]{../images/tiles_palette.png}
...
ROMs into RAM off a SD card. This would allow us to store multiple games on
...
beneficial to re-implement legacy systems in a FPGA. We believe that
re-implementation can be done at a reasonable cost being that the physical
hardware needed for a FPGA solution is cheaper than it has ever been before
...
In this project we showed possible methods and testing procedures for
reimplementing a legacy computer system. We found that it was relatively easy
to get the base system up and running which we attribute mostly to the fact
that one of the core components, the Zilog Z80 CPU, already existed as a proven
open source project. Trying to fully implement the Z80 would have been a huge
undertaking on its own and we doubt we would have even come close to
implementing the actual Game Gear system in the given time frame if we did not
leverage the TV80 project. We found that translating old design concepts, such
as tri-state buses, to new ones was a non-issue and our first attempt at
implementing the memory and IO multiplexer was successful. Using system level
simulation to verify the overall functionality of our design proved extremely
beneficial in identifying irregularities between our system and the emulator
that we tested against. Had we not had a proper emulator this project would
have been significantly more difficult. Using the emulator as a known `good'
system we were able to create video RAM dumps which we used to initialize the
video sub system while developing it. Had we not had this ability we would have
had to develop the video subsystem blind, not knowing if any video issues were
because of the CPU subsystem or the video subsystem. While there are still a
few functional limitations such as no sprites or controller input we were
successfully able to get commercial game ROMs running on our system proving
that our re-implementation was successful.

\newpage
\section{Code}
All code and other documentation associated with this project can be found here:\\
...
\begin{thebibliography}{9}
    ...
    \bibitem{de1} \url{http://www.altera.com/education/univ/materials/boards/de1/unv-de1-board.html}
\end{thebibliography}

\includepdf[pages={1}]{sig_sheet_1.pdf}
\includepdf[pages={1}]{sig_sheet_2.pdf}
